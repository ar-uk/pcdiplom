 ...
 * Modal form for saving a new build
 */
...
export const SaveBuildModal: React.FC<SaveBuildModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(buildName, description || undefined, isPublic)
      // Reset form
      setBuildName('')
      setDescription('')
      setIsPublic(false)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full shadow-lg">
        ...
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-lg font-bold text-gray-900">Save Your Build</h2>
          <p className="text-sm text-gray-600 mt-1">
            Give your build a name and optional description
          </p>
        </div>
